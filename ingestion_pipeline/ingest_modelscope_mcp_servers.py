...
from ingestion_pipeline.sources.mcp_pipeline import ModelScopeMCPPipeline  # noqa: E402
...
async def _run() -> None:
    ...
    # Optional cap via env: MODELSCOPE_MCP_TARGET (not part of SOP; fetch is by pages)
    target_raw = os.environ.get("MODELSCOPE_MCP_TARGET")
    target_count = None
    if target_raw:
        try:
            v = int(target_raw)
    ...
    pipeline = ModelScopeMCPPipeline(page_size=100, timeout=60.0, target_repo_count=target_count)
