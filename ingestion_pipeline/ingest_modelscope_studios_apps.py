...
from ingestion_pipeline.sources.modelscope_studios import (  # noqa: E402
...
async def _run() -> None:
    ...
    target_raw = os.environ.get("MODELSCOPE_STUDIOS_TARGET")
    target_count = None
    if target_raw:
        try:
            value = int(target_raw)
            if value >= 1:
                target_count = value
        except Exception:
            target_count = None

    pipeline = ModelScopeStudiosPipeline(
        studios_page_size=24,
        timeout=60.0,
