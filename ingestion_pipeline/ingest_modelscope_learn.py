...
from ingestion_pipeline.sources.modelscope_learn import (  # noqa: E402
    ModelScopeLearnPipeline,
)
...
async def _run() -> None:
    ...
    target = os.environ.get("LEARN_TARGET_COUNT")
    try:
        target_count = int(target) if target else None
    except Exception:
        target_count = None

    pipeline = ModelScopeLearnPipeline(page_size=18, timeout=60.0, target_repo_count=target_count)
    limits = PipelineRuntimeLimits(
        max_workers=8,
        max_embed_concurrency=8,
        ingest_batch_size=8,
        chunk_max_size=32768,
    )
